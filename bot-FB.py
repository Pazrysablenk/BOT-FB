...
try:
	...
	print("\x1b[0;93mkamu belum menginstall module rich ")
	print("\x1b[0;92mModule rich sedang diinstall ")
...
try:
	...
	print("\x1b[0;93mAnda belum menginstall module requests ")
	print("\x1b[0;92mModule requests sedang diinstall ")
...
try:
	...
	print("\x1b[0;93mAnda belum menginstall module bs4 ")
	print("\x1b[0;92mModule bs4 sedang diinstall ")
...
try:
	...
	print("\x1b[0;93mAnda belum menginstall module mechanize ")
	print("\x1b[0;92mModule mechanize sedang diinstall ")
...
try:
	...
	print("\x1b[0;93mAnda belum menginstall module futures ")
	print("\x1b[0;92mModule futures sedang diinstall ")
...
try:
	...
	print("\x1b[0;93mAnda belum menginstall module stdiomask ")
	print("\x1b[0;92mModule stdiomask sedang diinstall ")
...
{P}██████╔╝{P}╚██████╔╝   ██║      ██║     ██████╔╝
{P}╚═════╝  {P}╚═════╝    ╚═╝      ╚═╝     ╚═════╝ 
{K}CODE BY PAZRYSABLENK {P}[ {H}Versi 1.0.1 {P}]""")

## [ Garis² Untuk Mempercantik:v ] ##
...
def login():
	logo()
	jalan(maling_pangsit+" Selamat datang di sc "+H+"BOT-FB")
	print(P+" ["+O+"01"+P+"] Login menggunakan"+H+" cookie")
	print(P+" ["+O+"02"+P+"] Cara menggunakan script"+H+" BOT-FB")
	print(P+" ["+M+"00"+P+"] Exit")
	...
	if gk_nanya in ["1","01","a"]:
		...
		if tanya_balek in ["y","Y","ya","Yes","yes"]:
			jalan(maling_pangsit+" Terimakasih sudah menggunakan sc "+H+"BOT-FB")
			...
			jalan(maling_pangsit+" Isi yang benar sayang")
			login()
	else:
		jalan(maling_pangsit+" Isi yang benar kontol")
		login()
...
def cara_menggunakan():
	...
	if cok in ["chat","Chat","CHAT"]:
		jalan(maling_pangsit+" Ini nomor whastapp admin ─> "+H+"+6283109115523\n"+P+"Chat aja kalo ada kepentingan atau mau ditanyakan ke author"+H+" BOT-FB")
		input(maling_pangsit+" Enter untuk kembali")
		login()
...
def login_cookie():
	...
	cookie = str(input(maling_pangsit+" Masukkan 🍪 :"+H+" "))
...
def menu():
	...
	try:lok = x["locale"]
		...
		lok = M+"-"+P
	jalan(maling_pangsit+" Selamat datang [ " +H+nama+P+" ] Ngentod")
	...
	print(maling_pangsit+" TTL facebook [ %s%s%s ]"%(H,pko,P))
	...
	print("")
	jalan(maling_pangsit+" Kalo kena spam tunggu 1 hari baru bisa digunakan kembali ^_^ ")
	print(P+" ["+O+"01"+P+"] Bot share for facebook ")
	print(P+" ["+O+"02"+P+"] Bot komen for facebook ")
	#print(P+" ["+O+"03"+P+"] Bot komen profil for facebook ")
	#print(P+" ["+O+"04"+P+"] Bot komen grup for facebook ")
	#print(P+" ["+O+"05"+P+"] Bot komen public ")
	#print(P+" ["+O+"05"+P+"] Bot likes for facebook" )
	...
	if mattzy in ["1","01"]:
		...
		jalan(maling_pangsit+" Isi yang baleg kontol")
		menu()
...
def share():
	...
	with requests.Session() as xyz:
		try:
			...
			os.system("rm -rf token.txt")
			login()
	link = input(maling_pangsit+"masukan Link postingan public :"+H+" ")
	print(maling_pangsit+" Minimal"+H+" 14")
...
def komen_profil():
	try:
		...
		jalan(maling_pangsit+" Cookie basi")
		os.system('rm -rf cookie.txt')
		os.system('rm -rf token.txt')
		login()
